...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if in_between(x, seg.x1, seg.x2) and in_between(y, seg.y1, seg.y2)
      self
    else
      NoPoints.new
    end
...
class LineSegment < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(x1, x2)
      if y2 < seg.y2
        l = LineSegment.new(x1, y1, x2, y2)
        r = LineSegment.new(seg.x1, seg.y1, seg.x2, seg.y2)
      else
        l = LineSegment.new(seg.x1, seg.y1, seg.x2, seg.y2)
        r = LineSegment.new(x1, y1, x2, y2)
      end
      if real_close(l.y1, r.y2)
        Point.new(l.x1, l.y1)
      else
        if l.y1 < r.y2
          NoPoints.new
          ...
          if l.y1 > r.y1
            ...
            LineSegment.new(l.x1, l.y1, r.x2, r.y2)
          end
        end
      end
    else
